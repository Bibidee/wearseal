...
| `contracts/base/WearSealEscrow.sol` | Base Sepolia native-ETH escrow: relayer registration, exact renter funding, one-time full-collateral allocation, recipient pull claims, reentrancy protection, revert-preserving retry semantics and no administrator withdrawal | covered by the Base escrow suite; new source requires fresh deployment before READY |
