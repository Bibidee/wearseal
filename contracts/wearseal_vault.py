...
class WearsealVault(gl.Contract):
    ...
    def __init__(self, agreement_contract):
        ...
        assert str(address).lower() != ZERO
        self.agreement_contract = address; self.credited = u256(0); self.settled = False
    ...
    def deposit(self, amount, funding_tx):
        assert gl.message.value == 0 and self.credited == 0 and not self.settled and int(amount) > 0 and valid_tx_hash(funding_tx)
        a = Agreement(self.agreement_contract).view().get_agreement()
        assert gl.message.sender_address == a["renter"] and int(amount) == int(a["deposit"]) and a["status"] == "BASELINE_ACCEPTED"
    ...
    def ack_owner_claim(self, payout_tx):
        a = Agreement(self.agreement_contract).view().get_agreement(); assert self.settled and gl.message.sender_address == a["owner"] and self.owner_claim > 0 and not self.owner_claimed and valid_tx_hash(payout_tx)
        self.owner_claimed = True; self.owner_claim_tx = payout_tx

    @gl.public.write
    def ack_renter_claim(self, payout_tx):
        a = Agreement(self.agreement_contract).view().get_agreement(); assert self.settled and gl.message.sender_address == a["renter"] and self.renter_claim > 0 and not self.renter_claimed and valid_tx_hash(payout_tx)
        self.renter_claimed = True; self.renter_claim_tx = payout_tx

    @gl.public.write
    def refund_cancelled(self, payout_tx):
        assert self.credited > 0 and not self.settled and valid_tx_hash(payout_tx)
