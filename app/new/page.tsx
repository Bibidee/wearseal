...
export default function New() {
  ...
  const [serial, setSerial] = useState('serial-hash');
  const [url, setUrl] = useState(checkoutDefault);
  ...
  return <main className="wrap"><SiteNav/><div className="page-shell form-shell"><div className="kicker">NEW AGREEMENT · STUDIONET 61999</div><h1 className="page-title">Seal the baseline.</h1><p className="subhead">A five-step handoff record. Every value stays visible before deployment.</p><div className="stepper">{[1,2,3,4,5].map(x=><i className={`step-dot ${x<=step?'active':''}`} key={x}/>)}</div>
    {step===1&&<section className="form-card"><div className="eyebrow">STEP 01 / ITEM</div><h2>Identify the equipment.</h2><label className="form-label">ITEM NAME<input className="plate" value={label} onChange={e=>setLabel(e.target.value)} placeholder="WearSeal demo camera"/></label><label className="form-label">RENTER ADDRESS<input className="plate" value={renter} onChange={e=>setRenter(e.target.value)} placeholder="0x renter address"/></label><label className="form-label">SERIAL OR REFERENCE<input className="plate" value={serial} onChange={e=>setSerial(e.target.value)}/></label><button className="button" onClick={()=>setStep(2)}>CONTINUE →</button></section>}
    {step===2&&<section className="form-card"><div className="eyebrow">STEP 02 / CHECKOUT EVIDENCE</div><h2>Verify the exact image.</h2><label className="form-label">LOCAL CHECKOUT IMAGE<input className="plate" type="file" accept="image/*" onChange={e=>{setLocal(e.target.files?.[0]);setMatch(undefined);setImageHash('')}}/></label><label className="form-label">PUBLIC HTTPS URL<input className="plate" value={url} onChange={e=>setUrl(e.target.value)}/></label><button className="button secondary" onClick={verify}>FETCH + COMPARE BYTES</button>{imageHash&&<div className="tx-banner"><strong>{match?'MATCH · HASH VERIFIED':'MISMATCH · DEPLOYMENT BLOCKED'}</strong><HashDNA hash={imageHash}/><p className="mono">LOCAL {imageHash}</p></div>}<button className="button" disabled={match!==true} onClick={()=>setStep(3)}>LOCK EVIDENCE →</button></section>}
