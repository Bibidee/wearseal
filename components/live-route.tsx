  ...
  const run = async () => {
    try {
      ...
      if (action === 'fund') {
        ...
        const attested = await attestation.json() as {hash?: string; status?: string; readback?: boolean};
        if (attested.hash && attested.readback) setTx({phase: 'FINALIZED_SUCCESS', hash: attested.hash}); else setTx({phase: 'IDLE'});
  ...
  const runVaultAction = async (functionName: 'settle'|'claim_owner'|'claim_renter'|'refund') => {
    try {
      ...
      if (functionName === 'refund') {
        ...
        await wallet.switchBaseSepolia();
        const existingRefund = recovery?.refundTx as string | null | undefined;
        const refundHash = existingRefund || await sendBase(wallet.provider, wallet.account, claimData(agreementAddress)); if (!existingRefund) await waitBaseReceipt(wallet.provider, refundHash); setBaseTx(refundHash);
        await wallet.switchStudionet?.();
        ...
        const attested = await attestation.json() as {hash?: string; readback?: boolean}; if (attested.hash && attested.readback) setTx({phase: 'FINALIZED_SUCCESS', hash: attested.hash}); else setTx({phase: 'IDLE'}); await read(); return;
      }
      if (!wallet.switchBaseSepolia) throw Error('Base Sepolia wallet switching is unavailable.');
      ...
      await wallet.switchBaseSepolia();
      const existingClaim = (functionName === 'claim_owner' ? recovery?.ownerClaimTx : recovery?.renterClaimTx) as string | null | undefined;
      const hash = existingClaim || await sendBase(wallet.provider, wallet.account, claimData(agreementAddress)); if (!existingClaim) await waitBaseReceipt(wallet.provider, hash); setBaseTx(hash);
      await wallet.switchStudionet?.();
      ...
      const attested = await attestation.json() as {hash?: string; readback?: boolean};
      if (attested.hash && attested.readback) setTx({phase: 'FINALIZED_SUCCESS', hash: attested.hash}); else setTx({phase: 'IDLE'});
    } catch (e) { setError(describeError(e)); }
  };
  const runAllocation = async () => { try { setError(''); const response = await fetch('/api/escrow/relay', {method: 'POST', headers: {'content-type': 'application/json'}, body: JSON.stringify({agreement: agreementAddress})}); if (!response.ok) throw Error((await response.json()).error || 'Base Sepolia payout allocation failed.'); await read(); } catch (e) { setError(describeError(e)); } };
